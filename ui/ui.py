...
class UI:
    ...
    def display_client_menu():
        ...
        print("7. Sort clients")
        print("8. Exit client menu")
        print()
    ...
    def display_rental_menu():
        ...
        print("4. Display most rented movies")
        print("5. Display customers with most rentals")
        print("6. Display overdue rentals")
                ...
                while True:
                    ...
                    try:
                        if user_operation_command == '1':
                            ...
                            self.__sort_movies()
                        elif user_operation_command == '10':
                            break
                        else:
                            print("Not a valid number option. Choose between 1 and 5.")
                    except RepositoryException as repository_error:
...
#RECURSIV
    def __read_id(self):
        try:
            ...
            print('Client id must be an integer above 0!')
            return self.__read_id()
        return client_id #recursiv

    def __read_name(self):
        try:
            client_name=input("nume client:")
        except ValueError:
            print('Client name must be a string')
            return self.__read_name()
        return client_name #recursiv
    ...
    def __sort_movies(self):
        movies=[]
        for el in self.__movie_services.get_all_movies():
            movies.append(el)
        movies=self.__movie_services.insertionSort(movies)
        for movie in movies :
            print(movie)

    def __sort_clients(self):
        clients=[]
        for el in self.__client_services.get_all_clients():
            clients.append(el)
        clients=self.__client_services.insertionSortclients(clients)
        for client in clients :
            print(client)
