...
from exceptions_and_validators.validator import MovieValidator
...
class MovieServices:
    ...
    def compare(self,movie1,movie2):
        ...
        if movie1.get_description() != movie2.get_description():
            return movie1.get_description() < movie2.get_description()
        return movie1.get_genre() < movie2.get_genre()


    def insertionSort(self,lst, reverse=False):
        #best case O(n)
        #worst case O(n^2)
        #medium case O(n^2)
        if len(lst)<=1 :
