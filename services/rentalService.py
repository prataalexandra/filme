...
from domain.rental import RentalDTO, Rental, RentalMovieStatisticsDTO, RentalClientStatisticsDTO
from exceptions_and_validators.exception import RepositoryException
from exceptions_and_validators.validator import RentalValidator
...
class RentalServices:
    ...
    def generate_random_rental_data(self):
        ...
        for rental_index in range(21):
            ...
            returned_date = random.choice(
                [random_rental_date + datetime.timedelta(days=random.randint(1, 180)), '"n/a"'])
    ...
    def rent_new_movie(self, rental_id, movie_id, client_id, rental_date):
        ...
        rental = Rental(rental_id, movie_id, client_id, rental_date, test_due_date, '"n/a"')
        ...
        for each_rental in rentals_list:
            if each_rental.get_client_id() == client_id:
                ...
                if return_date_for_rental.date() > due_date_for_rental.date():
    ...
    def get_most_rented_movies(self):
        ...
        for rental in rentals:
            ...
            return_date = rental.get_returned_date() if rental.get_returned_date() != '"n/a"' else datetime.datetime.now()
            rental_date = datetime.datetime.strptime(rental_date, '%Y-%m-%d')
            if not isinstance(return_date, datetime.date):
                return_date = datetime.datetime.strptime(return_date, '%Y-%m-%d')
            number_of_rental_days = (return_date - rental_date).days

            if movie_id not in movie_id_and_days_counter:
                movie_id_and_days_counter[movie_id] = 0
            movie_id_and_days_counter[movie_id] += number_of_rental_days
        ...
        return most_rented_movies_list[:len(most_rented_movies_list) // 2 + 1]
    ...
    def get_most_active_clients(self):
        ...
        for rental in rentals:
            ...
            client_id_and_days_counter[client_id] += number_of_rental_days
        ...
        most_active_clients_list = RentalServices.insertionSort(most_active_clients_list,
                                                                key=lambda item: item.get_number_of_days())
        return most_active_clients_list[:len(most_active_clients_list) // 2 + 1]

    def get_mostrentedmovies(self):
        rentals = self.__rental_repository.get_all()
        ...
        for rental in rentals:
            ...
            if str(return_date) > str(due_date):
                number_of_late_rental_days = (return_date - due_date).days
        ...
        RentalServices.combSort(late_rental_movies_list,key=lambda item:item.get_number_of_days,reverse=True)
    ...
    def get_late_rentals_descending_order(self):
        ...
        for movie_id in movie_id_and_late_days_counter:
            ...
            number_of_days = movie_id_and_late_days_counter[movie_id]
            late_rental_movies_list.append(RentalMovieStatisticsDTO(movie_title, number_of_days))
        late_rental_movies_list=RentalServices.combSort(late_rental_movies_list,key=lambda item:item.get_number_of_days,reverse=True)
        return late_rental_movies_list
    ...
    def insertionSort(lst, key=lambda x: x, reverse=False):
        ...
        for i in range(1, len(lst)):
            ind = i - 1
            a = lst[i]
            while ind >= 0 and key(a) <= key(lst[ind]):
                lst[ind + 1] = lst[ind]
            ind = ind - 1
            lst[ind + 1] = a
        if reverse is True:
    ...
    @staticmethod
    def insertionSortRec(lst, key=lambda x: x, reverse=False, n=-1):
        if n <= 1:
            return
        RentalServices.insertionSortRec(lst, key, reverse, n - 1)
        last = lst[n - 1]
        j = n - 2

        while (j >= 0 and key(lst[j]) > key(last)):
            lst[j + 1] = lst[j]
            j = j - 1
            lst[j + 1] = last
        if reverse is True:
